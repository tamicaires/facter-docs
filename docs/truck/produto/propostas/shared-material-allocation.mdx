...
### 5. Fechamento de período — como o mês "fecha"? (Aguardando produto)
...
1. **Selo de status** — Aberto/provisório (○) vs. Fechado/oficial (●). Nunca se confunde um número provisório com um oficial.
2. **Carimbo de fechamento** — "Fechado em DD/MM por [usuário], base: X".
3. **Alerta de divergência** (fase 2) — se entrou lançamento depois do fechamento, aparece "há lançamentos posteriores → Reprocessar?". O sistema é honesto sobre a foto estar desatualizada.
...
**O que produto precisa decidir:** confirmar o modelo híbrido (C) como desenhado. Ele define o escopo das tasks #17 (motor), #18 (exibição com status) e da nova task de **fechamento de período**.
...
| Período/fechamento | Modelo híbrido (Opção C): mês aberto recalcula ao vivo, mês fechado vira snapshot oficial, com reprocessar. Detector de divergência = fase 2. Ver **Seção 5 (Decisões)** | **Recomendado: C** — aguardando produto |
