...
2. **Quantidade validada** — `returnedQuantity` deve ser >= 1 e <= `approvedQuantity`
3. **Estoque restaurado** — `Part.stockQuantity += returnedQuantity` (atomico, dentro de transaction)
...
|---------|---------------|-----------|
| Devolver 3 de 10 entregues | +3 no estoque, RETURNED | Validacao: qty <= approved |
